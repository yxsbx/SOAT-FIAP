...
public class UpdateWorkshopServiceUseCase {
    ...
    public WorkshopService execute(Command command) {
        ...
        workshopService.update(command.name(), command.description(), command.basePrice(), command.estimatedTimeInMinutes());
        if (command.active()) {
    ...
    public record Command(UUID serviceId, String name, String description, Money basePrice, int estimatedTimeInMinutes, boolean active) {
    }
}
