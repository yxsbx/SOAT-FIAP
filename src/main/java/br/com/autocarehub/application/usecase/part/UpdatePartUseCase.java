...
public class UpdatePartUseCase {
    ...
    public Part execute(Command command) {
        ...
        part.update(command.name(), command.sku(), command.category(), command.subcategory(), command.brand(), command.unitPrice(), command.minimumStock());
        if (command.active()) {
            part.activate();
        } else {
            part.deactivate();
    ...
    public record Command(UUID partId, String name, String sku, String category, String subcategory, String brand, Money unitPrice, int minimumStock, boolean active) {
    }
}
