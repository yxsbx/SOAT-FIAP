...
class ServiceOrderFlowIntegrationTest {
  ...
  private void addServiceToServiceOrder(String token, UUID serviceOrderId, UUID serviceId)
      throws Exception {
    mockMvc
        .perform(
            post("/api/v1/service-orders/{serviceOrderId}/services", serviceOrderId)
                .header("Authorization", bearer(token))
                .contentType(MediaType.APPLICATION_JSON)
  ...
  private void addPartToServiceOrder(String token, UUID serviceOrderId, UUID partId)
      throws Exception {
    mockMvc
        .perform(
            post("/api/v1/service-orders/{serviceOrderId}/parts", serviceOrderId)
                .header("Authorization", bearer(token))
                .contentType(MediaType.APPLICATION_JSON)
  ...
  private void generateBudget(String token, UUID serviceOrderId) throws Exception {
    mockMvc
        .perform(
            post("/api/v1/service-orders/{serviceOrderId}/budget/generate", serviceOrderId)
                .header("Authorization", bearer(token)))
        .andExpect(status().isOk())
  ...
  private void approveBudget(String token, UUID serviceOrderId) throws Exception {
    mockMvc
        .perform(
            post("/api/v1/service-orders/{serviceOrderId}/budget/approve", serviceOrderId)
                .header("Authorization", bearer(token)))
        .andExpect(status().isOk())
