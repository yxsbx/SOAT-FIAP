...
class AdministrativeCrudIntegrationTest {
    ...
    private UUID manageCustomer(String token) throws Exception {
        ...
        mockMvc
                .perform(
                        get("/api/v1/customers").param("active", "true").header("Authorization", bearer(token)))
                .andExpect(status().isOk())
    ...
    private void manageVehicle(String token, UUID customerId) throws Exception {
        ...
        mockMvc
                .perform(
                        get("/api/v1/vehicles/{vehicleId}", vehicleId).header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.plate").value("CRD1A23"));

        mockMvc
                .perform(
                        get("/api/v1/customers/{customerId}/vehicles", customerId)
                                .header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items").isArray());
        ...
        mockMvc
                .perform(
                        put("/api/v1/vehicles/{vehicleId}", vehicleId)
                                .header("Authorization", bearer(token))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(json(updateRequest)))
                .andExpect(status().isOk())
        ...
        mockMvc
                .perform(
                        delete("/api/v1/vehicles/{vehicleId}", vehicleId)
                                .header("Authorization", bearer(token)))
                .andExpect(status().isNoContent());
    }
    ...
    private void manageWorkshopService(String token) throws Exception {
        ...
        mockMvc
                .perform(
                        get("/api/v1/workshop-services/{serviceId}", serviceId)
                                .header("Authorization", bearer(token)))
                .andExpect(status().isOk())
        ...
        mockMvc
                .perform(
                        put("/api/v1/workshop-services/{serviceId}", serviceId)
                                .header("Authorization", bearer(token))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        json(
        ...
        mockMvc
                .perform(
                        delete("/api/v1/workshop-services/{serviceId}", serviceId)
                                .header("Authorization", bearer(token)))
                .andExpect(status().isNoContent());
    }
    ...
    private void managePart(String token) throws Exception {
        ...
        mockMvc
                .perform(get("/api/v1/parts/{partId}", partId).header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.sku").value("PAD-CRUD-001"));

        mockMvc
                .perform(
                        get("/api/v1/parts").param("active", "true").header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items").isArray());
        ...
        mockMvc
                .perform(
                        put("/api/v1/parts/{partId}", partId)
                                .header("Authorization", bearer(token))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(json(updateRequest)))
                .andExpect(status().isOk())
        ...
        mockMvc
                .perform(delete("/api/v1/parts/{partId}", partId).header("Authorization", bearer(token)))
                .andExpect(status().isNoContent());
    }
    ...
    private void manageDemoLead(String token) throws Exception {
        ...
        mockMvc
                .perform(get("/api/v1/demo-leads").header("Authorization", bearer(token)))
                .andExpect(status().isOk())
    ...
    private void manageUsersAndPreferences(String token) throws Exception {
                        ...
                        put("/api/v1/users/me/preferences/home")
                                ...
                                .content(
                                        json(
                                                Map.of(
                                                        "widgets",
                                                        List.of("orders-progress", "pending-budgets"),
                                                        "showAlertsOnHome",
                                                        true))))
                .andExpect(status().isOk())
        ...
        mockMvc
                .perform(get("/api/v1/users/me/preferences/home").header("Authorization", bearer(token)))
                .andExpect(status().isOk())
        ...
        mockMvc
                .perform(
                        get("/api/v1/users")
                                .param("search", "crud.employee")
                                .header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items[0].id").value(userId.toString()));

        mockMvc
                .perform(get("/api/v1/users/partners").header("Authorization", bearer(token)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items").isArray());
