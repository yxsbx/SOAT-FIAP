...
| RF-002 | Identificar cliente por CPF/CNPJ             | Localizar cliente por documento no fluxo da OS.                                     | A criação de OS aceita `customerDocument` e vincula o cliente correto.              | `CreateServiceOrderUseCase`, `CustomerRepository`, `docs/api/openapi/openapi.yaml`.            | Atendido |
...
| RF-006A | Preservar identidade do veículo             | Impedir alteração de placa, marca, modelo e ano em veículo já cadastrado.           | `PUT /api/v1/vehicles/{vehicleId}` permite quilometragem/status e bloqueia identidade. | `UpdateVehicleUseCase`, `AdministrativeCrudIntegrationTest`, `docs/api/openapi/openapi.yaml`. | Atendido |
...
| RF-018 | Listar Ordens de Serviço                     | Permitir consulta administrativa das OS.                                            | `GET /api/v1/service-orders` retorna lista filtrável.                               | `ListServiceOrdersUseCase`, `docs/api/openapi/openapi.yaml`.                                   | Atendido |
...
| RF-022 | Receber notificação externa de orçamento     | Registrar aprovação ou recusa recebida por webhook simulado.                        | Endpoints externos não exigem JWT e exigem `X-External-Service-Token`.              | `ServiceOrdersController`, `ExternalServiceTokenVerifier`, `ServiceOrderFlowIntegrationTest`. | Atendido |
| RF-023 | Receber notificação externa de status        | Atualizar status da OS por webhook de ferramenta externa simulada.                  | Status inválido, token externo inválido e transição inválida são bloqueados.         | `UpdateServiceOrderStatusUseCase`, `ServiceOrderFlowIntegrationTest`.                      | Atendido |
| RF-024 | Listar fila operacional priorizada           | Exibir OS abertas na ordem Execução, Aguardando Aprovação, Diagnóstico e Recebida.  | Finalizadas e entregues não aparecem na listagem operacional padrão.                | `ListServiceOrdersUseCase`, `ServiceOrderJpaRepository`, `ListServiceOrdersUseCaseTest`.   | Atendido |

## 5. Requisitos não funcionais

| ID      | Requisito não funcional          | Categoria      | Descrição                                                     | Critério de aceite                                                          | Evidência                                                      | Status   |
|---------|----------------------------------|----------------|---------------------------------------------------------------|-----------------------------------------------------------------------------|----------------------------------------------------------------|----------|
| RNF-001 | Backend monolítico               | Arquitetura    | Backend entregue como uma única aplicação Spring Boot.        | Existe uma aplicação principal `AutoCareHubApiApplication`.                 | `backend/pom.xml`, `backend/Dockerfile`, `backend/src/main/java`.                      | Atendido |
| RNF-002 | Arquitetura Hexagonal/Clean Architecture | Arquitetura    | Separar interface, aplicação, domínio, portas, adaptadores e infraestrutura. | Pacotes `interfaces`, `application`, `domain` e `infrastructure` existem, com portas em `application.port.out`. | Estrutura de pacotes e `docs/architecture/PHASE2_ARCHITECTURE.md`. | Atendido |
| RNF-003 | API RESTful                      | Arquitetura    | Expor recursos por endpoints REST versionados.                | Rotas `/api/v1/**` documentadas no OpenAPI.                                 | `docs/api/openapi/openapi.yaml`.                                   | Atendido |
...
| RNF-005 | Banco PostgreSQL                 | Persistência   | Usar PostgreSQL como banco principal do MVP.                  | Compose sobe `autocarehub-postgres` e backend usa JDBC PostgreSQL.          | `deploy/docker/docker-compose.yml`, `application.yml`.                       | Atendido |
| RNF-006 | Justificativa do banco           | Persistência   | Explicar a escolha por banco relacional.                      | Arquitetura documenta relações entre cliente, veículo, OS, itens e estoque. | `docs/architecture/ARCHITECTURE.md`.                                        | Atendido |
| RNF-007 | Dockerfile para build            | Execução local | Permitir build da API em container.                           | `docker build` consegue montar a imagem da API.                             | `backend/Dockerfile`.                                                  | Atendido |
| RNF-008 | Docker Compose completo          | Execução local | Subir banco, backend e frontend demonstrativo.                | `.\scripts\start-local.ps1 -Rebuild -Reset` sobe os serviços definidos.                  | `deploy/docker/docker-compose.yml`.                                          | Atendido |
| RNF-009 | Execução local simples           | Operação       | Projeto deve ter comandos claros para rodar localmente.       | README documenta `.env`, Docker Compose, Swagger e testes.                  | `README.md`, `deploy/docker/.env.example`.                                   | Atendido |
...
| RNF-012 | Testes unitários e de integração | Testabilidade  | Cobrir domínio, use cases, REST, segurança e fluxos críticos. | `mvn test` executa a suíte automatizada.                                    | `backend/src/test/java`, `docs/testing/TESTING.md`.                            | Atendido |
| RNF-013 | Cobertura mínima de 80%          | Qualidade      | Manter cobertura acima do mínimo exigido.                     | `mvn verify` passa no gate JaCoCo.                                          | `backend/pom.xml`, `backend/target/site/jacoco/index.html`.                    | Atendido |
| RNF-014 | Relatório de vulnerabilidades    | Segurança      | Consolidar scans executados e riscos reais.                   | Relatório lista ferramentas executadas, achados e riscos residuais aceitos. | `docs/security/SECURITY_REPORT.md`.                                     | Atendido |
| RNF-015 | `deploy/docker/.env.example` sem secrets reais | Segurança      | Exemplo de ambiente não pode conter credenciais reais.        | Arquivo usa placeholders para senha e JWT.                                  | `deploy/docker/.env.example`, `.gitignore`.                                  | Atendido |
| RNF-016 | Migrations com Flyway            | Persistência   | Schema deve ser versionado.                                   | Existe migration baseline e Flyway roda nos testes/execução.                | `backend/src/main/resources/db/migration`, logs do `mvn verify`.       | Atendido |
| RNF-017 | Login e senhas desacoplados      | Segurança      | Login deve autenticar e senhas devem usar hash seguro.        | Aplicação depende das portas `AuthenticationGateway` e `PasswordHasher`; infraestrutura implementa Spring Security/JWT e BCrypt. | `AuthenticationGateway`, `PasswordHasher`, `SpringSecurityAuthenticationGateway`, `BCryptPasswordHasher`. | Atendido |
...
| RNF-002   | Pacotes em camadas                                     | Não se aplica                                                  | Estrutura `backend/src/main/java/br/com/autocarehub`                           | Architecture                        |
...
| RNF-008   | Docker Compose                                         | `deploy/docker/docker-compose.yml`                                           | `docker compose config --quiet`                                        | README                              |
| RNF-009   | README e `deploy/docker/.env.example`                                | Comandos locais                                                | Validação documental                                                   | README                              |
...
| RNF-012   | Testes automatizados                                   | `backend/src/test/java`                                                | `mvn test`                                                             | Testing                             |
| RNF-013   | JaCoCo                                                 | `mvn verify`                                                   | `backend/target/site/jacoco`                                                   | Testing, Static Analysis            |
| RNF-014   | Scans de segurança                                     | Não se aplica                                                  | `docs/security/SECURITY_REPORT.md`                                              | Security Report                     |
| RNF-015   | `deploy/docker/.env.example`, `.gitignore`                           | Variáveis locais                                               | Arquivos revisados                                                     | Security Report                     |
| RNF-016   | Flyway                                                 | `backend/src/main/resources/db/migration`                              | Logs de migration nos testes                                           | Architecture                        |
