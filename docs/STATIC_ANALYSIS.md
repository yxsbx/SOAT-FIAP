...
Ele complementa o `docs/SECURITY_REPORT.md`: o relatório de segurança consolida vulnerabilidades e riscos residuais,
...
| Maven/JUnit            | `mvn clean install` aprovado com 149 testes, 0 falhas, 0 erros e 0 ignorados.          |
| JaCoCo                 | Gate aprovado. Cobertura: 96,02% de instruções, 96,66% de linhas e 90,12% de branches. |
...
90% foi mantido no `pom.xml`; a correção foi feita cobrindo cenários reais que já existiam no código.

Também foi removida do `pom.xml` a configuração de Testcontainers e o override de `commons-compress` em escopo de teste,
...
- alinhamento com `docs/SECURITY_REPORT.md`, que permanece como documento oficial de vulnerabilidades e riscos
  residuais.
...
`target/generated-sources/openapi`, indicando uso de `@Valid` em listas geradas. Como o código é gerado a partir do
...
Os arquivos em `security-reports/` ficam versionados como evidência revisada da entrega. A pasta `target/` permanece
