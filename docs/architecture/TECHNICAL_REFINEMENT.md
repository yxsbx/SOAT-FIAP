...
| Preservar identidade do veículo  | Update bloqueia placa, marca, modelo e ano.         | Evita que correções cadastrais sobrescrevam histórico de OS de outro veículo. | `UpdateVehicleUseCase`, `docs/api/openapi/openapi.yaml`.                |
...
| Cobertura mínima       | JaCoCo no `mvn verify`.                                                   | Garante cobertura acima da exigência mínima do projeto.                              | `backend/pom.xml`, `backend/target/site/jacoco`.                           |
| Vulnerabilidades       | Dependency-Check, npm audit, Semgrep, Gitleaks, Docker Scout e OWASP ZAP. | Registra riscos encontrados, tratados e aceitos.                                     | `docs/security/SECURITY_REPORT.md`.                                 |
...
| Como documentar a API?                                  | Versionar o OpenAPI em `docs/api/openapi/openapi.yaml`.                                               | O contrato documenta endpoints e apoia geração de interfaces.                              |
...
| Cobertura JaCoCo                  | Garantir cobertura acima do mínimo.                                               | `mvn verify` passa no gate de cobertura.                | `backend/target/site/jacoco`.                                        |
| Scans de segurança                | Consolidar vulnerabilidades e riscos aceitos.                                     | Resultado documentado separadamente.                    | `docs/security/SECURITY_REPORT.md`.                                   |
