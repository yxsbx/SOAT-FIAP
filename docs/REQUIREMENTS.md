...
| RF-006A | Preservar identidade do veículo             | Impedir alteração de placa, marca, modelo e ano em veículo já cadastrado.           | `PUT /api/v1/vehicles/{vehicleId}` permite quilometragem/status e bloqueia identidade. | `UpdateVehicleUseCase`, `AdministrativeCrudIntegrationTest`, `docs/openapi/openapi.yaml`. | Atendido |
...
| RNF-002 | Arquitetura Hexagonal/Clean Architecture | Arquitetura    | Separar interface, aplicação, domínio, portas, adaptadores e infraestrutura. | Pacotes `interfaces`, `application`, `domain` e `infrastructure` existem, com portas em `application.port.out`. | Estrutura de pacotes e `docs/PHASE2_ARCHITECTURE.md`. | Atendido |
