...
| RNF-017 | Senhas com BCrypt                | Segurança      | Senhas de usuários devem usar hash seguro.                    | Aplicação depende da porta `PasswordHasher` e infraestrutura implementa BCrypt. | `PasswordHasher`, `BCryptPasswordHasher`, `SecurityConfig`.    | Atendido |
...
| RNF-017   | `PasswordHasher`, `BCryptPasswordHasher`, `SecurityConfig` | Login/usuários                                             | BCrypt via porta de aplicacao                                         | Security Report                     |
