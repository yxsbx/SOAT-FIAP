...
| 2:30-4:00   | Execução local com Docker.                                       | `.\scripts\start-local.ps1 -Rebuild -Reset`, `docker compose --env-file .env -f docker-compose.yml ps`.                                                                                                                         | Docker e Compose.                |
...
| 11:00-12:30 | CI/CD.                                                           | `.github/workflows/quality.yml`, `.github/workflows/phase2-ci-cd.yml`, `docs/CI_CD.md`, tela do GitHub Actions e secrets necessários sem exibir valores.                                                                        | Pipeline automatizada.           |
...
- Manifests em `k8s/` aplicáveis no cluster escolhido.
...
- Se os secrets nao estiverem configurados, explicar no video que o deploy Kubernetes foi pulado/simulado por limitação de ambiente.
- Link do video e PDF final preenchidos na entrega.

## Demonstração Kubernetes

1. Abrir `k8s/README.md` e mostrar a separação dos manifests.
...
2. Abrir `.github/workflows/phase2-ci-cd.yml` e mostrar:
   - `mvn -B verify`;
   - `npm ci`, lint e build;
