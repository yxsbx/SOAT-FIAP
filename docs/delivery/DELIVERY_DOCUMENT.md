...
A branch final de entrega é `main`. O acesso de leitura ao repositório privado foi confirmado no GitHub para o usuário avaliador `soat-architecture` antes do envio no portal.
...
- listagem operacional de OS movida para query no repository/adapter, ordenada por status e data, ocultando OS finalizadas e entregues;
...
é aprovação de orçamento, controle de status, estoque e consulta pelo cliente.
