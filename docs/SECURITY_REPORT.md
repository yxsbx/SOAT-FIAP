...
A análise considera os resultados das ferramentas realmente executadas no projeto: OWASP Dependency-Check, npm audit, Docker Scout, OWASP ZAP, Semgrep, Gitleaks e JaCoCo. Os resultados abaixo não foram estimados manualmente; eles foram consolidados a partir dos relatórios gerados localmente. O detalhamento de qualidade, cobertura e análise estática fica em `docs/STATIC_ANALYSIS.md`.
...
| DAST API | Analisado | OWASP ZAP API scan executado contra `/v3/api-docs`, com 0 falhas e 2 avisos revisados.                                        |
...
A análise dinâmica com OWASP ZAP foi executada como evidência complementar contra a API local. O relatório continua tratando Dependency-Check, npm audit, Docker Scout, Gitleaks, Semgrep e JaCoCo como os gates principais da entrega.

## 3.1 Segurança desde o desenho da solução

O AutoCare Hub trata dados pessoais e operacionais de uma oficina: CPF/CNPJ de clientes, telefone, e-mail, dados de veículos, credenciais de usuários administrativos, tokens JWT, estoque, orçamentos e Ordens de Serviço. Por isso, a validação de segurança foi considerada nos fluxos principais do MVP, e não apenas no scan final.

Os controles aplicados no projeto são:

- validação de CPF/CNPJ no value object `Document`, com cálculo real dos dígitos verificadores;
...
- `.env` e `target/` fora do versionamento; `security-reports/` versionado como evidência revisada da entrega;
...
- tratamento global de erros sem retorno intencional de stacktrace;
- CORS sem wildcard e com origens configuráveis;
- headers de segurança configurados no Spring Security;
- Docker com runtime backend distroless/non-root e compose com `read_only`/`no-new-privileges`;
- scans de dependências, secrets, imagens Docker e análise estática.

Ficaram fora do escopo principal do MVP: auditoria produtiva, WAF, rotação automatizada de secrets e observabilidade de segurança em produção. O frontend demonstrativo usa `localStorage` para o JWT, decisão aceitável para o escopo local acadêmico; por isso o projeto evita renderização HTML insegura no frontend e mantém CSP/headers no backend.
...
| Command Injection | Backend de negócio | O backend não executa comandos do sistema operacional no fluxo do MVP | Busca por `Runtime.getRuntime` e `ProcessBuilder` sem ocorrência |
| XSS em campos textuais | Campos renderizados no frontend demonstrativo | Vue escapa interpolação por padrão; não há `v-html`, `innerHTML`, `eval` ou APIs equivalentes no frontend | Busca estática no diretório `frontend/` |
...
| Erro interno expondo detalhe técnico | Responses de erro da API | `RestExceptionHandler` padroniza resposta e não retorna stacktrace | `RestExceptionHandler` |
...
No estado final da entrega, não há vulnerabilidades críticas ou altas abertas nos scans consolidados. Os riscos residuais documentados são duas CVEs médias em imagens Docker: uma no backend por dependência transitiva sem versão corrigida disponível no Maven Central, e uma no frontend por BusyBox sem versão corrigida indicada pelo Docker Scout.
...
O JaCoCo mede domínio, aplicação, controllers REST, segurança, mappers e adapters de persistência. Foram excluídos apenas o bootstrap da aplicação, classes geradas automáticamente pelo OpenAPI e records estruturais de comando, consulta e saída sem lógica própria.
...
| VULN-012 | Imagem backend | Substituição da imagem runtime Ubuntu/Temurin por distroless Java 21 non-root. | `Dockerfile`, `docker-compose.yml` | Scout atual sem críticas/altas e com 1 média residual. |
...
| Prevenção contra Command Injection | Implementado por redução de superfície | Backend não executa comandos do sistema operacional no fluxo do MVP. |
| Prevenção contra XSS no frontend demonstrativo | Validado | Sem uso de `v-html`, `innerHTML`, `eval` ou APIs equivalentes; Vue escapa interpolação por padrão. |
...
| Insecure Design | Segurança tratada apenas após o desenvolvimento | Validações no domínio, papéis, avaliação de ameaças e gates de segurança | Este relatório, testes de segurança e domínio |
| Security Misconfiguration | CORS amplo, Swagger produtivo exposto, container privilegiado | CORS sem wildcard, Swagger configurável, Docker non-root/read-only | `SecurityConfig`, `application.yml`, Dockerfile e compose |
...
| Security Logging and Monitoring Failures | Exposição de dados sensíveis em erro/log | Handler global sem stacktrace na resposta; Semgrep/Gitleaks sem achados | `RestExceptionHandler`, Semgrep, Gitleaks |
...
A análise principal de bibliotecas foi feita por OWASP Dependency-Check no backend e `npm audit` no frontend. O projeto também foi revisado para remover dependências de teste que não eram usadas pela suíte real, reduzindo superfície de dependências e a quantidade de artefatos analisados no scan final.
...
Não há plugin de relatório automático de licenças configurado no Maven. Se for necessário anexar uma evidência formal de licença, o guia de scans mostra um comando complementar para gerar relatório local; esse resultado só deve ser citado depois de executado.
...
A imagem backend ficou sem vulnerabilidades críticas ou altas e mantém 1 CVE média em `jackson-databind`, aceita temporariamente porque a versão corrigida indicada pelo Docker Scout ainda não está disponível no Maven Central. A imagem frontend também ficou sem vulnerabilidades críticas ou altas e mantém 1 CVE média em BusyBox, aceita temporariamente porque o Docker Scout não indicou versão corrigida para a base analisada. Esses riscos foram registrados, mitigados com configuração de container mais restritiva e direcionados para revisão na próxima atualização de dependências/imagens.

Considerando o escopo analisado por Dependency-Check, npm audit, Docker Scout, OWASP ZAP, Gitleaks, Semgrep e JaCoCo, o sistema está apto para a entrega acadêmica do Tech Challenge.
