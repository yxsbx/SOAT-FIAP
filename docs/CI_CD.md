...
| Secret | Obrigatório para deploy real | Uso |
| ------ | ---------------------------- | --- |
...
O push para GHCR usa `GITHUB_TOKEN`, secret automático do GitHub Actions. Não configure nem exponha tokens pessoais para esse fluxo sem necessidade.
...
Se `KUBE_CONFIG`, `POSTGRES_PASSWORD`, `JWT_SECRET` ou `EXTERNAL_SERVICE_TOKEN` não estiverem configurados, a pipeline continua executando build, testes, validações e build das imagens. O deploy real no cluster é pulado com mensagem explícita no log.

Esse comportamento evita expor credenciais e permite demonstrar a automação mesmo sem cluster disponível.
...
- A mensagem de deploy pulado se o ambiente não tiver secrets.

Não afirme que houve deploy real se o job apenas executou as etapas de build/validação ou pulou o Kubernetes por falta de secrets.
